...
def student_home(request):
    ...
    total_subject = Matiere.objects.filter(filiere=student.filiere).count()
    ...
    data_present = []
    data_absent = []
    subjects = Matiere.objects.filter(filiere=student.filiere)
    for subject in subjects:
        attendance = Attendance.objects.filter(matiere=subject)
...
def student_view_attendance(request):
    ...
    if request.method != 'POST':
        filiere = get_object_or_404(Filiere, id=student.filiere.id)
        context = {
            'subjects': Matiere.objects.filter(filiere=filiere),
        ...
        try:
            ...
            attendance = Attendance.objects.filter(
                date__range=(start_date, end_date), matiere=subject)
...
def student_list_cours(request):
    ###
    ...
    return render(request, "student_template/student_list_cours.html", context)


def list_absence(request):
    student = get_object_or_404(Student, admin=request.user)
    allAbsence = AddAbsence.objects.filter(student=student)
    ...
    return render(request, "student_template/student_list_absence.html", context)


def list_edt(request):
    allEdt = Edt.objects.all()
    paginator = Paginator(allEdt, 3)  # 3 actus par page

    page_number = request.GET.get('page')  # Récupérer le numéro de la page
    page_obj = paginator.get_page(page_number)  # Obtenir les objets pour la page actuelle
    context = {
        'page_obj': page_obj,
        'page_title': 'Liste des EDT'
    }

    return render(request, "student_template/student_list_edt.html", context)
